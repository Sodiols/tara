/*
TARA MIGRATION 0006 -- Fix: no order could ever be placed (pgcrypto + English-only note)

Run after 0005_english_only_storefront.sql. Safe to re-run.

THE BUG
-------
Checkout failed for every customer, every time, with the generic
"Your order could not be placed. No payment was taken."

place_order() declared its tracking token as:

    new_tracking_token text := encode(gen_random_bytes(24), 'hex');

gen_random_bytes() is supplied by the pgcrypto extension, which Supabase
installs into the `extensions` schema. place_order() is `security definer` and
therefore correctly runs with `set search_path = ''`, so the unqualified name
could not be resolved and PostgreSQL raised:

    42883: function gen_random_bytes(integer) does not exist

That happens in the DECLARE block -- before the customer, address, stock,
coupon and cash-on-delivery checks ever run -- so the failure was total and
unconditional. Cash on delivery was never actually rejected; the function died
before it got that far. (gen_random_uuid() nearby kept working because it is a
pg_catalog built-in, not a pgcrypto function, which is why the rest of the
schema looked healthy.)

THE FIX
-------
Build the token from two v4 UUIDs instead. gen_random_uuid() is a pg_catalog
built-in from PostgreSQL 13 onward, so it resolves under an empty search_path
and requires no extension. The token keeps the same shape the application and
the tracking page already expect: 48 lowercase hex characters, 192 bits of
entropy.

ALSO IN THIS MIGRATION
----------------------
The initial tracking event no longer writes a Bangla note. `note_bn` is
nullable and is simply left unset; the column and every existing value stay
exactly as they are.

WHAT IS NOT CHANGED
-------------------
Nothing else. Pricing, stock locking and deduction, coupon validation and
usage counting, idempotency, duplicate-order and rate-limit protection, the
cash-on-delivery rule and the notification outbox are all byte-for-byte the
behaviour established in migration 0004.
...
Before this migration that raised 42883 (gen_random_bytes does not exist).
After it, it raises `invalid_customer_or_address` -- proof that execution now
reaches the validation block.
*/
