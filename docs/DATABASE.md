...
| `0016_guest_tracking_details.sql` | Adds `get_guest_order_tracking()`: returns one order snapshot only when the order number **and** its tracking token both match, so an order number alone reveals nothing. RLS on the underlying tables is unchanged. |
...
| `0018_oauth_profile_metadata.sql` | Makes `handle_new_user()` read Google's identity metadata (`name` as well as `full_name`, and so on), so a Google sign-up creates a complete profile instead of a blank one. Required for Google sign-in. |
...
| `0023_archive_and_trash.sql` | **Run before deploying the matching code** — the admin screens read the new `archived_at` columns. Adds Archive & Trash: the admin-only `archive.manage` permission, `archived_at` on categories, collections and reviews, the `archived_items` registry (kept in sync by triggers and logged to `admin_audit_log`), and `admin_archive_item()`, `admin_restore_archived_item()` and `admin_purge_archived_item()`. Only administrators can restore or purge; purge refuses anything tied to orders or still in use. Revokes direct DELETE on products, categories, collections and product_variants. |
