...
- `npx playwright test e2e/collection-editorial.spec.ts --reporter=list`: 15 passing cases across Chromium desktop and Pixel 7 emulation. Three intentional skips avoid running mouse-only, touch-only, or viewport-matrix cases in the wrong profile.
...
- No-JavaScript content and decoded first image remain useful. Reduced motion removes movement; elapsed time and page scrolling do not change the selected collection.
